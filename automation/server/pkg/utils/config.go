...
const (
	DisabledComment CommentsTagFlag = "disabled_comment" //关闭注释
	HeadComment     CommentsTagFlag = "head_comment"     //注释在上
	LineComment     CommentsTagFlag = "line_comment"     //注释在同一行
	FootComment     CommentsTagFlag = "foot_comment"     //注释在下
)
...
func newOptions(opts ...Option) *Options {
	opt := &Options{
		CommentsTagFlag: DisabledComment,
		OmitEmpty:       true,
...
func (e *YamlOpeartor) Marshal() ([]byte, error) {
	if e.options.CommentsTagFlag == DisabledComment {
...
func addCommentPosition(node *yaml.Node, commentValue string, tagFlag CommentsTagFlag) {
	if tagFlag.enabled() {
		...
		case DisabledComment:
			fmt.Println("disabled comment tag!")
		case HeadComment:
		...
		default:
			fmt.Println("no support comment tag!")
		}
